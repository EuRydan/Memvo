...
const PLANS = [
  ...
  },
  {
    ...
    features: [
      ...
      { text: 'Até 7 desafios fotográficos', included: true },
      { text: 'Álbum em tempo real', included: true },
      { text: 'Download do QR Code', included: true },
      { text: 'Download em ZIP', included: true },
      { text: 'Upload de vídeos (1 min)', included: true },
      { text: 'Álbum Livre (Sem desafios)', included: false },
      { text: 'Suporte prioritário', included: false },
    ],
  },
  {
    ...
    features: [
      ...
      { text: 'Download em ZIP', included: true },
      { text: 'Upload de vídeos (3 min)', included: true },
        ...
        <div className="text-center mb-10">
          ...
          {userEvents.length > 0 && (
            <div className="inline-flex flex-col items-center mb-4">
              <label className="text-xs font-semibold text-stone mb-2">Para qual evento?</label>
              <div className="relative">
                <select
                  value={eventId || ''}
                  onChange={(e) => setEventId(e.target.value)}
                  className="appearance-none bg-canvas border border-hairline text-ink text-sm font-semibold rounded-lg pl-4 pr-10 py-2 focus:outline-none focus:ring-2 focus:ring-ink/20 focus:border-ink cursor-pointer shadow-sm min-w-[200px]"
                >
                  {userEvents.map(ev => (
          ...
          <div className="flex flex-nowrap overflow-x-auto items-stretch justify-start xl:justify-center gap-6 lg:gap-8 mb-14 pt-16 pb-16 snap-x snap-mandatory px-6 sm:px-12" style={{ scrollbarWidth: 'none' }}>
            {PLANS.filter(plan => !( (activeCoupon || isValidatingCoupon) && plan.id === 'freemium')).map(plan => {
              ...
              >
                ...
                <h1 className={`text-4xl font-bold mt-2 ${plan.popular ? 'text-white' : displayPrice === 'R$0' ? 'text-gray-400' : 'text-[#0a0a0a]'}`}>
