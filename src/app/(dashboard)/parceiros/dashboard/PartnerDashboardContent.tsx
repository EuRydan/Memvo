...
export function PartnerDashboardContent({ affiliateCode }: { affiliateCode: string }) {
  ...
  return (
    <div className="flex flex-col gap-2">
      <button 
        onClick={handleCopy}
        className="text-xs font-semibold bg-ink text-canvas px-4 py-2 rounded-full hover:opacity-90 transition-opacity"
      >
        {copied ? t('dashboard.copied') : t('dashboard.copyLink')}
      </button>
      <button 
        onClick={handleWhatsApp}
        className="text-xs font-semibold bg-[#25D366] text-white px-4 py-2 rounded-full hover:bg-[#128C7E] transition-colors flex items-center justify-center gap-1.5"
      >
